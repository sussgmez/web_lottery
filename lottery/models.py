...
class Order(models.Model):
    ...
    lottery = models.ForeignKey(
        "lottery.Lottery", verbose_name=_("Sorteo"), on_delete=models.CASCADE
    )
    ...
    dollar = models.ForeignKey(
        "lottery.Dollar",
        verbose_name=_("Tasa del dolar"),
    ...
    payment_method = models.IntegerField(
        _("Método de pago"), choices=PAYMENT_METHOD, default=0
    )

    # Pago móvil
    dni = models.IntegerField(_("Cédula de identidad"), blank=True, null=True)
    type_of_dni = models.IntegerField(
        _("Tipo de DNI"), choices=TYPE_OF_DNI, blank=True, null=True
    )
    ...
    bank_code = models.CharField(
        _("Banco"), max_length=4, choices=BANK_CODE, blank=True, null=True
    )
    reference = models.IntegerField(_("Nro. de referencia"), blank=True, null=True)

    # Zelle/Paypal
    email_or_phone = models.EmailField(
        _("Email o teléfono"), max_length=254, blank=True, null=True
    )
    ...
    def __str__(self):
        return (
            f"{self.pk}: {self.user.email} lottery-{self.lottery.pk} ({self.quantity})"
        )
    ...
    if created:
        while len(instance.tickets.all()) < instance.quantity:
            available_numbers = set(
                [x for x in range(1, (Lottery.AVAILABLE_TICKETS + 1))]
            ) - set([x.number for x in Ticket.objects.all()])
            Ticket.objects.create(
                order=instance, number=random.choice(list(available_numbers))
            )
